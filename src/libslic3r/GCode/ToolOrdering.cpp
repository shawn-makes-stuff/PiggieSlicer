        ...
        for (const LayerRegion *layerm : layer->regions()) {
            ...
            if (! layerm->perimeters.entities.empty()) {
                ...
                if (something_nonoverriddable){
                    // PiggieSlicer / FullSpectrum: resolve virtual mixed ids per layer
                    unsigned int outer_wall_fid = mf_resolve((extruder_override == 0) ? (unsigned int) region.config().outer_wall_filament_id.value : (unsigned int) extruder_override);
               		layer_tools.extruders.emplace_back(outer_wall_fid);
                    ...
                    if (layerCount == 0) {
                        firstLayerExtruders.emplace_back((int) outer_wall_fid);
                    }
                }
            ...
            if (something_nonoverriddable || !m_print_config_ptr) {
            	if (extruder_override == 0) {
                    ...
                    if (has_internal_solid)
                        layer_tools.extruders.emplace_back(mf_resolve((unsigned int) region.config().internal_solid_filament_id));
                    if (has_top_solid_surface)
                        layer_tools.extruders.emplace_back(mf_resolve((unsigned int) region.config().top_surface_filament_id));
                    if (has_bottom_surface)
                        layer_tools.extruders.emplace_back(mf_resolve((unsigned int) region.config().bottom_surface_filament_id));
	                if (has_infill)
	                    layer_tools.extruders.emplace_back(mf_resolve((unsigned int) region.config().sparse_infill_filament_id));
