...
void Sidebar::priv::flush_printer_sync(bool restart)
{
    ...
    //btn_sync_printer->SetBackgroundColorNormal((*counter_sync_printer & 1) ? "#F8F8F8" :"#EC6FA6");
...
{
    StateColor not_synced_colour(std::pair<wxColour, int>(wxColour("#EC6FA6"), StateColor::Normal));
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        struct PanelColors {
            ...
            wxColour bg_focus  = "#FCEAF2";
            wxColour bd_normal = "#DBDBDB";
            wxColour bd_hover  = "#EC6FA6";
            wxColour bd_focus  = "#EC6FA6";
        };
        ...
        */
        ...
        StateColor btn_sync_bd_col(
                std::pair<wxColour, int>(wxColour("#EC6FA6"), StateColor::Pressed),
                std::pair<wxColour, int>(wxColour("#EC6FA6"), StateColor::Hovered),
    ...
    text_ctrl->Bind(wxEVT_SET_FOCUS, [this](wxFocusEvent& e) {
        ...
        p->m_search_bar->SetBorderColor(wxColour("#EC6FA6"));
...
void Sidebar::sync_ams_list(bool is_from_big_sync_btn)
{
    ...
    if (!unknowns.empty()) {
        MessageDialog dlg(this,
            _L("There are some unknown or incompatible filaments mapped to generic preset.\nPlease update PiggieSlicer or restart PiggieSlicer to check if there is an update to system presets.") + detail,
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
    if (!sync_color_only) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if(load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3MF is not supported by PiggieSlicer, loading geometry data only."), _L("Load 3MF"));
                    }
                    ...
                    //             show_info(q, _L("The 3MF was generated by an old PiggieSlicer, loading geometry data only."), _L("Load 3MF"));
                    //         else
                    //             show_info(q, _L("The 3MF is not supported by PiggieSlicer, loading geometry data only."), _L("Load 3MF"));
                    //     }
                    ...
                    //     }
                    // }
                    // Orca: check if the project is created with PiggieSlicer 2.3.1-alpha and use the sparse infill rotation template for non-safe infill patterns
                        ...
                        if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                            ...
                            if (!is_safe_to_rotate) {
                                wxString msg_text = _(
                                    L("This project was created with an PiggieSlicer 2.3.1-alpha and uses "
                                ...
                                msg_text += "\n\n" +
                                            _(L("Would you like PiggieSlicer to automatically fix this by clearing the rotation template settings?"));
                        ...
                        load_config = false;
                        show_info(q, _L("The 3MF file was generated by an old PiggieSlicer version, loading geometry data only."), _L("Load 3MF"));
                    }
                    else if (!load_config) {
    ...
    if (preset_type == Preset::TYPE_FILAMENT && sidebar->is_multifilament()) {
        ...
        if (preset_type == Preset::TYPE_PRINTER) {
            ...
            if (combo->is_selected_printer_model()) {
                ...
                if (preset == nullptr) {
                    // PiggieSlicer: no concrete system preset exists for this printer model (e.g. the
                    // vendor's presets failed to load). Abort the selection instead of dereferencing
                    // a null pointer (was an ACCESS_VIOLATION crash on selecting a printer-model entry).
                    BOOST_LOG_TRIVIAL(error) << "on_select_preset: no system preset found for printer model \"" << preset_name << "\"";
                    return;
                }
                preset->is_visible = true; // force visible
...
#else
    wxGetApp().mainframe->SetTitle(m_project_name + " - PiggieSlicer");
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "PiggieSlicer window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to PiggieSlicer failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
...
void Plater::on_filament_count_change(size_t num_filaments)
{
    ...
    const size_t paint_filament_count = std::min(get_filament_count_with_mixed(),
                                                 static_cast<size_t>(EnforcerBlockerType::ExtruderMax));
    for (ModelObject* mo : wxGetApp().model().objects) {
        for (ModelVolume* mv : mo->volumes) {
            mv->update_extruder_count(paint_filament_count);
...
static void sync_mixed_filaments_from_project(PresetBundle *pb, const std::vector<std::string> &phys)
{
    ...
    std::string defs;
    if (const auto *o = pb->project_config.option<ConfigOptionString>("mixed_filament_definitions"))
        defs = o->value;

    // Keep the UI-side manager synchronized with project_config. This is called from render
    // paths, so skip the rebuild when inputs and manager state are already coherent.
    ...
    if (type == PrinterWarningType::NOT_CONNECTED) {
        if (device_page) {
            ...
            content = wxString::Format(
                _L("PiggieSlicer can't connect to %s. Please check if the printer is powered on and connected to the network."), printer_name);
        }
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on PiggieSlicer(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
