...
public:
    ...
    // PiggieSlicer / FullSpectrum: physical extruder colours followed by the virtual
    // mixed-filament display colours, so painted mixed regions render with their blend
    // colour everywhere (3D view, thumbnails) — not just inside the paint gizmo.
