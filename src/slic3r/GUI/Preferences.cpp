...
private:
    void OnPaint(wxPaintEvent& evt)
    {
        ...
        wxColour textCol = StateColor::darkModeColorFor(m_hovered ? "#F291BC" : "#363636");
...
wxBoxSizer* PreferencesDialog::create_item_link_association( wxString url_prefix, wxString website_name)
{
    ...
    wxString tooltip = _L("Associate") + " " + url_prefix + ":// " + _L("with PiggieSlicer so that PiggieSlicer can open models from") + " " + website_name;
...
void PreferencesDialog::create_items()
{
    ...
    auto item_single_instance  = create_item_checkbox(_L("Allow only one PiggieSlicer instance"),
    #if __APPLE__
    ...
    #else
            _L("If this is enabled, when starting PiggieSlicer and another instance of the same PiggieSlicer is already running, that instance will be reactivated instead."),
    #endif
            "single_instance");
    ...
    auto item_remember_printer = create_item_checkbox(_L("Remember printer configuration"), _L("If enabled, PiggieSlicer will remember and switch filament/process configuration for each printer automatically."), "remember_printer_config");
    ...
    auto item_auto_reslice = create_item_auto_reslice(
        ...
        _L("If enabled, PiggieSlicer will re-slice automatically whenever slicing-related settings change."),
    ...
    auto item_stealth_mode     = create_item_checkbox(_L("Stealth mode"), _L("Keep network features limited to local/LAN operation. Profiles and presets stay on this computer; use manual export/import to move them between machines."), "stealth_mode");
    ...
    if (is_running_in_msix()) {
        ...
        g_sizer->Add(create_item_title(_L("Associate files to PiggieSlicer")), 1, wxEXPAND);
    ...
    auto item_associate_3mf    = create_item_checkbox(_L("Associate 3MF files to PiggieSlicer"), _L("If enabled, sets PiggieSlicer as default application to open 3MF files.") , "associate_3mf");
    g_sizer->Add(item_associate_3mf);

    auto item_associate_drc = create_item_checkbox(_L("Associate DRC files to PiggieSlicer"), _L("If enabled, sets PiggieSlicer as default application to open DRC files."), "associate_drc");
    g_sizer->Add(item_associate_drc);

    auto item_associate_stl    = create_item_checkbox(_L("Associate STL files to PiggieSlicer"), _L("If enabled, sets PiggieSlicer as default application to open STL files.") , "associate_stl");
    g_sizer->Add(item_associate_stl);

    auto item_associate_step   = create_item_checkbox(_L("Associate STEP files to PiggieSlicer"), _L("If enabled, sets PiggieSlicer as default application to open STEP files."), "associate_step");
    g_sizer->Add(item_associate_step);

    //// ASSOCIATE > WebLinks
    g_sizer->Add(create_item_title(_L("Associate web links to PiggieSlicer")), 1, wxEXPAND);
