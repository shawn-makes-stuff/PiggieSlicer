    ...
    : DPIDialog(static_cast<wxWindow*>(wxGetApp().mainframe), wxID_ANY, _L("Network plug-in update"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    operation_tips = new ::Label(this, Label::Body_12, _L("Click OK to update the Network plug-in when PiggieSlicer launches next time."), LB_AUTO_WRAP);
...
UpdateVersionDialog::UpdateVersionDialog(wxWindow *parent)
    : DPIDialog(parent, wxID_ANY, _L("New version of PiggieSlicer"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER)
{
    SetBackgroundColour(*wxWHITE);
...
void UpdateVersionDialog::update_version_info(wxString release_note, wxString version)
{
    ...
    if (is_running_in_msix())
        m_text_up_info->SetLabel(wxString::Format(_L("New version available: %s. Please update PiggieSlicer from the Microsoft Store."), version));
    else
    ...
    auto style    = "body {color:" + fg_color + "; background-color:" + bg_color + "; font-family:sans-serif}"
                  + "a    {color: #EC6FA6}"               // matches hyperlink colors
...
void ConfirmBeforeSendDialog::edit_cancel_button_txt(const wxString& txt, bool switch_green)
{
    ...
    if (switch_green)
    {
        StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(0, 137, 123), StateColor::Pressed),
                                std::pair<wxColour, int>(wxColour(242, 145, 188), StateColor::Hovered),
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    comfirm_before_enter_text = _L("1. Please confirm PiggieSlicer and your printer are in the same LAN.");
    ...
    m_test_right_msg->SetForegroundColour(wxColour(242, 145, 188));
