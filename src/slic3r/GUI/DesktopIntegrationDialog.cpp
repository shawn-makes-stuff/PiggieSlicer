...
bool DesktopIntegrationDialog::is_integrated()
{
    ...
    // confirmation that PiggieSlicer.desktop exists
    struct stat buffer;   
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        ...
        if (contains_path_dir(target_candidates[i], "images")) {
            ...
            std::string dest_path = GUI::format("%1%/images/%2%PiggieSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
    ...
    if(target_dir_icons.empty()) {
        BOOST_LOG_TRIVIAL(error) << "Copying PiggieSlicer icon to icons directory failed.";
    } else 
    	// save path to icon
        app_config->set("desktop_integration_icon_slicer_path", GUI::format("%1%/images/%2%PiggieSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i)
    {
        if (contains_path_dir(target_candidates[i], "applications")) {
            ...
            if (create_desktop_file(path, desktop_file)){
                BOOST_LOG_TRIVIAL(debug) << "PiggieSlicer.desktop file installation success.";
                break;
            } else {
            	// write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to PiggieSlicer.desktop file installation failed. failed path: " << target_candidates[i];
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        ...
        if (!target_dir_icons.empty())
        {
            ...
            std::string dest_path = GUI::format("%1%/images/%2%PiggieSlicer-gcodeviewer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
            if (copy_icon(icon_path, dest_path))
                // save path to icon
        ...
        std::string desktop_file = GUI::format(
            ...
            "GenericName=3D Printing Software\n"
            "Icon=PiggieSlicer-gcodeviewer%2%\n"
        ...
        if (create_desktop_file(desktop_path, desktop_file))
            ...
            show_error(nullptr, _L("Performing desktop integration failed - could not create Gcodeviewer desktop file. PiggieSlicer desktop file was probably created successfully."));
        }
    }
...
void DesktopIntegrationDialog::perform_downloader_desktop_integration(std::string url_prefix)
{
    ...
    std::string desktop_file_downloader = GUI::format(
        "[Desktop Entry]\n"
        "Name=PiggieSlicer URL Protocol %1% %2%\n"
        "Exec=%3% %%u\n"
        "Terminal=false\n"
        "Type=Application\n"
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        if (contains_path_dir(target_candidates[i], "applications")) {
            ...
            if (create_desktop_file(path, desktop_file_downloader)) {
                ...
                BOOST_LOG_TRIVIAL(debug) << "PiggieSlicerURLProtocol.desktop file installation success.";
                break;
            }
            else {
                // write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to PiggieSlicerURLProtocol.desktop file installation failed. failed path: " << target_candidates[i];
                target_dir_desktop.clear();
            }
        }
    }
    ...
    if (create_desktop_file(desktop_path, desktop_file_downloader)) {
        ...
        std::string command = GUI::format("xdg-mime default PiggieSlicerURLProtocol-%1%%2%.desktop x-scheme-handler/%1%", url_prefix, version_suffix);
